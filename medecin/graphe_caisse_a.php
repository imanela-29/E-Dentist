...
if(!isset($_SESSION["email"])){
  ...
  include 'php/conn.php';
    ...
    $qu1 = "SELECT month(date) as m, SUM(montant) as prix1 FROM paiement WHERE date >= DATE_FORMAT(NOW() ,'%Y-01-01') group by m";
...
<head>
  ...
  <title>Graphe caisse /an | Cabinet docteur</title>
  <!-- Favicon -->
                    ...
                    if($resChe > 0){
                      while($row = mysqli_fetch_assoc($res)){
                        ...
                        <tr class="text-center">
                          ...
                          <td class="text-success">'.$row["prix1"].'</td>
                        </tr>';
                      }
                    }
                  ?>
...
<script>
  ...
  $(document).ready(
    function(){
      setInterval(function(){
        $("#refresh_caisse_a").load("refresh_caisse_a.php");
      },1000);
    });
</script>