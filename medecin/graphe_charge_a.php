...
if(!isset($_SESSION["email"])){
  ...
  $stmt = "SELECT month(date_charge) as m, SUM(prix_charge) as total FROM charge WHERE date_charge >= DATE_FORMAT(NOW() ,'%Y-01-01') GROUP BY m";
...
<head>
  ...
  <title>Graphe charge /an | CFE</title>
  <!-- Favicon -->
            ...
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
                ...
                <tbody id="">
                  ...
                  <?php
                    $qu = "SELECT id, charge, situation, sum(prix_charge) as p, month(date_charge) as m FROM charge where date_charge >= DATE_FORMAT(NOW() ,'%Y-01-01') group by date_charge ORDER BY id DESC";
                    $res = mysqli_query($conn,$qu);
                    $resChe = mysqli_num_rows($res);
                    if($resChe > 0){
                      while($row = mysqli_fetch_assoc($res)){
