...
if(!isset($_SESSION["email"])){
  ...
  include 'php/conn.php';
    ...
    $qu1 = "SELECT date, SUM(montant) as prix1 FROM paiement WHERE date >= DATE_FORMAT(NOW() ,'%Y-%m-01') group by date ";
...
<head>
  ...
  <title>Graphe caisse /mois | Cabinet docteur</title>
  <!-- Favicon -->
                ...
                <tbody>
                  <?php
                    ...
                    if($resChe > 0){
                      while($row = mysqli_fetch_assoc($res)){
                        echo '
                        <tr role="" class="odd text-center">
                          <td>'.$row["date"].'</th>
                          <td class="text-success">'.$row["total"].'</td>
                          </tr>';
                      }
                    }
                  ?>
 ...
 $(document).ready(
    function(){
      setInterval(function(){
        $("#refresh_caisse_m").load("refresh_caisse_m.php");
      },1000);
    });
