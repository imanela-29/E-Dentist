                ...
                <div class="media align-items-center">
                  ...
                  <div class="media-body  ml-2  d-none d-lg-block">
                    ...
                    <?php
                      ...
                      if($resChe > 0){
                        while($row = mysqli_fetch_assoc($res)){
                          echo '<span class="mb-0 text-sm font-weight-bold">'.$row["nom"].'</span>';
                        }
                      }
        ...
        <div class="col-lg-6">
          <div class="card">
            ...
            <div class="card-header">
              <!-- Title -->
              <div class="row">
                <h5 class="h3 mb-0 col-sm-10">Aujourd'hui: <small class="text-warning"><?php echo $date = date('d-m-Y'); ?></small></h5>
              </div>  
            </div>
            <!-- Card body -->
            <div class="card-body p-0">
              <!-- List group -->
              <ul class="list-group list-group-flush" data-toggle="checklist" id="">
                <?php
                  $todos = $conn->query("SELECT checked, title, id, CAST(start_event AS TIME) as t FROM events WHERE CAST(start_event AS DATE) = CAST(NOW() AS DATE) order by t");
                        ...
                        <div class="checklist-item checklist-item-danger">
                          <div class="checklist-info">
                            <div class="custom-control custom-checkbox custom-checkbox-success">
                              <input class="custom-control-input" id="chk-todo-task-'.$todo["id"].'" type="checkbox" name="event" data-todo-id ="'.$todo['id'].'"
                               checked >
                              <label class="custom-control-label" for="chk-todo-task-'.$todo["id"].'"></label>
                              <h5 class="item-checked checklist-title mb-0">'.$todo["title"].'</h5>
                              <small class="item-checked">'.$todo["t"].'</small>
                            </div>
                          </div>
                          <div>
                          </div>
                        ...
                        <div class="checklist-item checklist-item-danger">
                          <div class="checklist-info">
                            <div class="custom-control custom-checkbox custom-checkbox-success">
                              <input class="custom-control-input" id="chk-todo-task-'.$todo["id"].'" type="checkbox" name="event" data-todo-id ="'.$todo['id'].'">
                              <label class="custom-control-label" for="chk-todo-task-'.$todo["id"].'"></label>
                              <h5 class="checklist-title mb-0">'.$todo["title"].'</h5>
                              <small>'.$todo["t"].'</small>
                            </div>
                          </div>
                          <div>
        ...
        <div class="col-lg-6">
          <div class="card">
            ...
            <div class="card-header">
              <!-- Title -->
              <div class="row">
                <h5 class="h3 mb-0 col-sm-10">Demain: <small class="text-primary"><?php echo $date = date('d-m-Y', strtotime('+1 day')); ?></small></h5>
              </div>  
            </div>
            <!-- Card body -->
            <div class="card-body p-0">
              ...
              <ul class="list-group list-group-flush" data-toggle="checklist" id="">
                <?php
                  $todos = $conn->query("SELECT checked, title, id, CAST(start_event AS TIME) as t FROM events WHERE CAST(start_event AS DATE) = CAST((NOW()+INTERVAL 1 day) AS DATE) order by t");
                  while($todo = $todos->fetch_assoc()) {
                      echo '
                      <li class="checklist-entry list-group-item flex-column align-items-start py-4 px-4">';
                    if($todo['checked']){
